...
#funds = data['user_id'].value_counts().index
#funds = [272]
...
# подсчёт количества всех дней периода (с учетом добавленных нулевых)
days_per_user_id = data_daily_agg_filled_added.groupby('user_id')['date'].nunique()

# подсчёт количества транзакций
tx_per_user_id = data['user_id'].value_counts()

# объединеняем данные по user_id
user_id_stats = pd.DataFrame({
    'num_days_orig': days_per_user_id_orig,
    'num_tx': tx_per_user_id,
    'num_days': days_per_user_id,
    'activity_ratio': tx_per_user_id / days_per_user_id
})

min_days = 150     # минимум 150 дней активности
min_tx = 1000      # минимум 1000 транзакций

'''
# фильтрация по комбинированному порогу - количество дней + количество транзакций
funds = user_id_stats[
    (user_id_stats['num_days'] >= min_days) &
    (user_id_stats['num_tx'] >= min_tx)
].index.tolist()
'''

'''
# фильтрация по комбинированному порогу - количество дней + соотношение количества платежей к количеству дней (всех)
funds = user_id_stats[
    (user_id_stats['activity_ratio']>=2) &
    (user_id_stats['num_tx'] >= min_tx)
].index.tolist()
'''


# фильрация по количеству активных дней
funds = user_id_stats[
    (user_id_stats['num_days_orig'] >= min_days) &
    (user_id_stats['num_tx'] >= min_tx)
].index.tolist()
